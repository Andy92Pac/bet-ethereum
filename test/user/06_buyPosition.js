...
contract('SocialBet', (accounts) => {
	...
	it("should buy position", async () => {
		...
		oldNbPositions = await instance.m_nbPositions.call();

		await instance.deposit({from: user, value: web3.utils.toWei('1', 'ether')});

		await instance.buyPosition(nbPositions, web3.utils.toWei('1', 'ether'), {from: user});

		nbPositions = await instance.m_nbPositions.call();

		assert.equal(parseInt(oldNbPositions) + 1, parseInt(nbPositions));
	});
