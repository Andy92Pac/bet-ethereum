...
contract('SocialBet', (accounts) => {
	...
	it("should abort because offer doesn't belong to sender", async () => {
		...
		await exceptions.catchRevert(instance.closeOffer(nbOffers, {from: admin}));
	...
	it("should close offer", async () => {
		...
		offerAmount = parseInt(offer._amount.toString());
		...
		assert.equal(newBalance, offerAmount + oldBalance);
